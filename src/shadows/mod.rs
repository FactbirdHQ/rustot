...
/// Trait for Reported types generated by `#[shadow_node]` and `#[shadow_root]`.
///
/// Provides:
/// - Flat union serialization for adjacently-tagged enum reported types
/// - KV persistence for `report_only(persist)` fields
///
/// ## Flat Union Serialization
///
...
/// ## KV Persistence
///
/// `report_only(persist)` fields are not part of the Delta type (cloud can't set them),
/// but are persisted to KV storage. When these fields are reported via
/// `Shadow::update_reported()`, their values are automatically written to KV.
///
...
pub trait ReportedFields: Serialize {
    ...
    ///
    /// The default implementation is a no-op — types without `report_only(persist)`
